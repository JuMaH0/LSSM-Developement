(function (I18n, $) {
    ...
    function renderSettings() {
        ...
        $.each(sortable, function () {
            ...
            markup = "";
            markup += '<div id="' + moduleKey + '_wrap"' + (module.cols ? 'class="row"' : "") + '>';
            if (!module.cols) {
                ...
                $.each(module.cols, function() {
                    let col = this;
                    markup += '<div class="col-md-' + Math.round(12/module.cols.length) + '" id="' + moduleKey + "_" + col.id + '_wrap">' +
                        '<h4>' + col.name +
                        // We will need to implement Reset of multicolumn later
                        // '<button class="btn btn-default settings-reset" data-module="' + moduleKey + '_' + col.id + '" style="margin-left: 5px;" type="reset"><span class="glyphicon glyphicon-floppy-remove" title="' + I18n.t('lssm.managedsettings.reset_btn') + '"></span></button>' +
                        '</h4>';
                    if (col.info_text) {
                        markup += '<h5 id="' + moduleKey + '_' + col.id +  '_description">' + col.info_text + '</h5>';
                    }
                    markup += '</div>';
                });
            }
            ...
            if (!module.cols) {
                ...
                $.each(module.cols, function () {
                    let col = this;
                    for (let settingsKey in col.settings) {
                        if (col.settings[settingsKey].ui.parent) {
                            $('#' + col.settings[settingsKey].ui.parent + '_' + col.id + '_wrap').append(renderUIElement(moduleKey, settingsKey, col
                                .settings[settingsKey]));
                        } else {
                            $('#' + moduleKey + '_' + col.id + '_wrap').append(renderUIElement(moduleKey, settingsKey, col.settings[settingsKey]));
                        }
                    }
                });
            }
        ...
        for (let moduleKey in lssm.managedSettings.registeredModules) {
            ...
            if (!module.cols) {
                ...
                $.each(module.cols, function () {
                    let col = this;
                    for (let settingsKey in col.settings) {
                        let setting = col.settings[settingsKey];
                        if (setting.ui.custom_function && setting.ui.custom_function_event) {
                            $('#' + moduleKey + '_' + settingsKey).on(setting.ui.custom_function_event, setting.ui.custom_function);
                        }
                    }
    ...
    function applyModuleFunctions() {
        ...
        $('#lssm-export-settings').click(function () {
            ...
            // Add managedSettings
            for (let moduleKey in lssm.managedSettings.registeredModules) {
                if (!lssm.managedSettings.registeredModules[moduleKey].cols) {
                    ...
                    $.each(lssm.managedSettings.registeredModules[moduleKey].cols, function() {
                        if (lssm.settings.get(moduleKey + "_" + this.id)) {
                            exportData.moduleSettings[moduleKey + "_" + this.id] = lssm.settings.get(moduleKey + "_" + this.id);
                        }
                    });
                }
            }
    ...
    function renderUIElement(moduleKey, settingsKey, element) {
        ...
        var incompatibel = false;
        // $(lssm.Module).each(function() {
        //     console.log($(this));
        //     if ($(this).active == true) {
        //         console.log($(this)[0].name.de);
        //     }
        // });
        if (element.ui.type === "radio") {
            ...
            response += '<div style="margin-left: 16px;"><input type="checkbox" ' + checked +
                ' style="margin-right: 4px;" name="' + elementName + '" id="' + elementName + '"' + ((incompatibel == true) ? ' disabled' : '') + '>' + element.ui.label + '</div>';
