...
(($, win, I18n) => {
    ...
    // $(function() {$('body').append('<script>let checkCombinationsOld=checkCombinations;checkCombinations=function(o){if(console.log("Test"),$("textarea").is(":focus"))return!0;checkCombinationsOld(o)};</script>')});
    ...
    let aaoText = '';
    let markup = '<div class="alert alert-warning" id="missionHelper"><a class="pull-right" id="changeText"><i class="glyphicon glyphicon-wrench"></i></a>';

    if (missionHelp.length > 0) {
        let missionId = missionHelp.attr('href').split("/").pop().replace(/\?.*/, '');
        let mission = lssm.missions[missionId];
        if (mission) {
 ...
    $('#changeText').click(function() {
        $(this).hide();
        $(this).after('<div id="changeMissionHelper"><h3>Text ändern</h3><small>Du kannst hierbei HTML verwenden (Alle Scripts werden rausgefiltert).</small><div id="changeMissionHelperTexts"></div><br><button class="btn btn-danger" id="abortChangeMissionHelper">Abbechen</button><button class="btn btn-success" id"confirmChangeMissionHelper">Speichern</button></div>');
        let rows = aaoText.split('<br>');
        $('#changeMissionHelperTexts').append(rows);
        $('#abortChangeMissionHelper').click(function() {
            $('#changeMissionHelper').remove();
            $('#changeText').show();
        });
    });
