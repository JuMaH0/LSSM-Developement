(function (I18n, $) {
    ...
    function setSettings() {
        ...
        let managedSettings = {
            ...
            "cols": [
                {
                    "name": I18n.t('lssm.renameFzDefaults.names.vehicleTypes'),
                    "id": "vehicleTypes",
                    "info_text": I18n.t('lssm.renameFzDefaults.descriptions.vehicleTypes'),
                    "settings": {
                        "renameFzVehicleShowHideButton": {
                            ...
                            "ui": {
                                ...
                                "custom_function": function () {
                                    $('[id^=' + LSS_RENAMEFZ_STORAGE + '_renameFz_vehicleTypes-]').toggle();
                                }
                            }
                        }
                    }
                ...
                {
                    "name": I18n.t('lssm.renameFzDefaults.names.stations'),
                    "id": "stations",
                    "info_text": I18n.t('lssm.renameFzDefaults.descriptions.stations'),
                    "settings": {
                        "renameFzStationsShowHideButton": {
                            ...
                            "ui": {
                                ...
                                "custom_function_event": "click",
                                "custom_function": function () {
                                    $('[id^=' + LSS_RENAMEFZ_STORAGE + '_renameFz_stations-]').toggle();
                                }
                            }
                        }
                    }
        ...
        let defaultShortings = DEFAULT_SHORTINGS_DE;
        ...
        $.each(defaultShortings, function (key, val) {
            ...
            $.extend(managedSettings.cols[0].settings, tmpObject);

        });

        $.each(lssm.get_buildings(), function (key, station) {
            if (I18n.t('lssm.renameFzDefaults.validStationTypes')[station.stationType]) {
                let tmpObject = {
                    ['renameFz_stations-' + station.stationId]: {
                        "default": station.stationName,
                        "ui": {
                            "label": station.stationName + " (" + I18n.t('lssm.renameFzDefaults.validStationTypes')[station.stationType] + "):&nbsp;",
                            "type": "text",
                            "description": ""
                        }
                    }
                };
                $.extend(managedSettings.cols[1].settings, tmpObject);
            }
        });
    ...
    function getSetting(col, setting) {
        let returnedSetting = lssm.managedSettings.getSetting(LSS_RENAMEFZ_STORAGE, setting, col);
        console.log(col + ' (' + setting + ')', returnedSetting);
    ...
    function getVehiclesSorted() {
        try {
            ...
            for (i = 0; i < $("#vehicle_table tbody tr").length; i++) {
                ...
                let vehicleType = $("#vehicle_table tbody tr")[i].children[0].children[0].attributes["vehicle_type_id"].value;
                ...
                vehicles["Building " + buildingIDs[i - 1]]["Type " + vehicleType].sort(function (a, b) {
                    return a - b
                });
            }
            return vehicles
    ...
    function rename() {
        try {
            ...
            if (!set.rename) {
                ...
                $.ajax({
                    url: '/vehicles/' + $('.vehicle_edit_button:first').attr('vehicle_id') + '/editName',
                    success: function (d) {
    ...
    function setOptionsForVehicle(e) {
        ...
        if (lssm.carsById[vehicleType]) {
            ...
            set.option.tagging = getSetting("vehicleTypes", "renameFz_vehicleTypes-" + vehicleType);
        }
        ...
        set.option.stationAlias = JSON.parse(localStorage["lssm_" + LSS_RENAMEFZ_STORAGE + "_stations"])["renameFz_stations-" + tr.find('td').eq(3).find('a').attr('href').replace(/\D*/g, '')];
        return {'id': id, 'vehicleType': vehicleType, 'oldName': set.option.old};
    }

    function showForms() {
        let data = setOptionsForVehicle($(this));
        if (data.oldName != replaceString().substr(0, 40)) {
            $(this).hide();
        }
        if (replaceString().length > 40) {
            $("#" + prefix + "_nameToLongDiv").show();
            $("#" + prefix + "_nameToLongTableBody").append("<tr><td>" + data.oldName + "</td><td>" + replaceString() + "</td><td>" + replaceString().substr(0, 40) + "</td></tr>");
            creatForm(data.id, replaceString().substr(0, 40), data.oldName);
        } else {
            creatForm(data.id, replaceString(), data.oldName);
        }
    }
    ...
    function createSettings() {
        ...
        $('#' + prefix + '_saveAll').click(function () {
            ...
            for (let i = 1; i < $("input.btn.btn-success").length; i++) {
